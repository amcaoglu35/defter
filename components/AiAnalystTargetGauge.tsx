...
export function AiAnalystTargetGauge({ company, report }: AiAnalystTargetGaugeProps) {
  const targetMetrics = useMemo(() => {
    ...
    const fairVal = report?.fairValue || price * 1.25;
    const targetPrice = report?.targetPrice12M || price * 1.3;

    const lowTarget = parseFloat((price * 0.95).toFixed(2));
  ...
  const { price, lowTarget, avgTarget, highTarget, upsidePct, isPositiveUpside, currency } = targetMetrics;
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-[var(--mist)]">
              OpenBB WallStreet Consensus &amp; Target Price Tracker
            </p>
          </div>
        </div>
